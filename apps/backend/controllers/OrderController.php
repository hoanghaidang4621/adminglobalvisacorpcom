...
use GlobalVisa\Models\VisaOrderMember;
...
use GlobalVisa\Repositories\OrderDetail;
use GlobalVisa\Repositories\OrderMember;
use GlobalVisa\Repositories\OrderReceipt;
...
use GlobalVisa\Repositories\UserSiteInfoCorp;
use GlobalVisa\Repositories\UserType;
...
class OrderController extends ControllerBase
{
    public function indexAction()
    {
        ...
        $sql_table = "FROM GlobalVisa\Models\VisaOrder  o
                LEFT JOIN GlobalVisa\Models\VisaReceipt r ON r.receipt_order_id = o.order_id";
    ...
    public function exportcsvAction()
    {
        ...
        $sql_table = "FROM visa_order o
                    ...
                    LEFT JOIN new_country c ON c.country_id = m.member_country_id
                    LEFT JOIN (SELECT order_id,
                                SUM(CASE WHEN order_fee_item_id = 321 THEN order_service_fee END )  service_sim_fee,
                                SUM(CASE WHEN order_fee_item_id = 322 THEN order_service_fee END )  service_insurance_fee
    ...
    public function viewAction(){
        ...
        switch ($temp['order_status']) {
            case VisaOrder::STATUS_SUCCESS:
                $temp['order_button_html'] = '<a href="'.$this->url->get('receipt/'.$order_model->getFormattedId()).'" target="_blank" class="btn btn-success">Receipt</a>';
                break;
        ...
        $temp['order_method'] = $this->my->getMethod($temp['order_payment_method'], $temp['order_cardtype'], $temp['order_isenrolled3d'], $temp['order_ispassed3d'], true);
        ...
        foreach ($order_detail as $detail){
            $extraService = ExtraService::findFirstById($detail->getOrderFeeItemId());
            if($extraService) {
                $temp['order_extra_services'][] = [
