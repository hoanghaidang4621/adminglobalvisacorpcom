...
class CountryController extends ControllerBase
{
    ...
    private function getParameter($keyword, $langCode)
    {
        ...
        $arrParameter = array();
        if ($langCode === $this->globalVariable->defaultLanguage) {
            $sql = "SELECT p.* FROM GlobalVisa\Models\VisaCountry p WHERE 1";
            if (!empty($keyword)) {
            ...
            $sql = "SELECT p.*, pl.* FROM GlobalVisa\Models\VisaCountry p 
                    INNER JOIN \GlobalVisa\Models\VisaCountryLang pl ON pl.country_id  = p.country_id  AND  pl.country_lang_code  = :lang_code:                           
                    WHERE 1";
