...
class GovernmentFee extends Component
{
    public static function findFirstById($visa_type_id,$country_code) {
        return NewGovernmentFee::findFirst([
            'fee_visatype_id = :TypeID: AND fee_country_code = :Countrycode:',
    ...
    public static function getGovernmentFee($visa_type_id,$country_code){
        $result ="";
        $govfee = self::findFirstById($visa_type_id,$country_code);
        if($govfee){
            $result = $govfee->getFeeValue();
        }
        return $result;

    }
    public static function getGovernmentNote($visa_type_id,$country_code){
        $result ="";
        $govfee = self::findFirstById($visa_type_id,$country_code);
        if($govfee){
            $result = $govfee->getFeeNote();
        }
        return $result;
